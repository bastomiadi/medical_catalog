<?php
/**
 * ICD-9 Procedure API Class
 * 
 * Provides REST API access to ICD-9-CM Procedure data from clinicaltables.nlm.nih.gov
 */

require_once __DIR__ . '/../Translator.php';

class Icd9ProcedureApi {
    private $config;
    private $translator;
    
    // API Base URL
    const SEARCH_API_BASE = 'https://clinicaltables.nlm.nih.gov/api/icd9cm_sg/v3/search';
    ...
    /**
     * Search ICD-9 Procedure codes by keyword
     * 
    ...
    /**
     * Search ICD-9 Procedure by keyword
     * 
    ...
    private function makeApiRequest($url) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'ICD-9-Procedure-PHP-Client/1.0');
        ...
        if ($error) {
            error_log("ICD-9 Procedure API Error: " . $error);
        }
        
        if ($httpCode !== 200) {
            error_log("ICD-9 Procedure API HTTP Error: " . $httpCode . " for URL: " . $url);
