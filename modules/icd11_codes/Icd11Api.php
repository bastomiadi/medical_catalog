<?php
/**
 * ICD-11 Codes API Class
 * 
 * Provides REST API access to ICD-11 Codes data from clinicaltables.nlm.nih.gov
 */
...
class Icd11Api {
    ...
    // API Base URL
    const SEARCH_API_BASE = 'https://clinicaltables.nlm.nih.gov/api/icd11_codes/v3/search';
    ...
    public function getByCode($code) {
        $params = [
            ...
            'sf' => 'code',
            'count' => 1,
            'ef' => 'code,title,definition,type,chapter'
        ];
    ...
    private function makeApiRequest($url) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'ICD-11-PHP-Client/1.0');
        ...
        if ($error) {
            error_log("ICD-11 API Error: " . $error);
        }
        
        if ($httpCode !== 200) {
            error_log("ICD-11 API HTTP Error: " . $httpCode . " for URL: " . $url);
    ...
    private function parseSearchResponse($response, $params) {
        ...
        for ($i = 0; $i < count($codes); $i++) {
            // Display format: [["code", "title"], ...]
            $title = '';
            if (isset($display[$i])) {
                if (is_array($display[$i])) {
                    $title = $display[$i][1] ?? $display[$i][0] ?? '';
                } else {
                    $title = $display[$i];
                }
            }
            ...
            $row = [
                ...
                'type' => $extra['type'][$i] ?? '',
                'chapter' => $extra['chapter'][$i] ?? ''
            ];
            
            // Add extra fields (only if extra is not null)
            if ($extra && is_array($extra)) {
                foreach ($extra as $field => $values) {
                    if (isset($values[$i]) && !in_array($field, ['definition', 'type', 'chapter'])) {
                        $row[$field] = $values[$i];
                    }
                }
            }
