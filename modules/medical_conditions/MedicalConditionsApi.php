<?php
/**
 * Medical Conditions API Class
 * 
 * Provides REST API access to Medical Conditions data from clinicaltables.nlm.nih.gov
 * Based on Regenstrief Institute's Medical Gopher program with over 2,400 medical conditions.
 */

class MedicalConditionsApi {
    protected $baseUrl;
    protected $apiKey;
    ...
    public function __construct($config) {
        $this->baseUrl = $config['api_url'] ?? 'https://clinicaltables.nlm.nih.gov/api/conditions/v3/';
     ...
     * @return array|null Condition data or null if not found
     */
    ...
    protected function makeRequest($url) {
        $cacheKey = md5($url);
        $cacheFile = sys_get_temp_dir() . '/medical_conditions_' . $cacheKey . '.json';
        ...
        // Handle clinicaltables.nlm.nih.gov API response format: [total, codes, extra, display, code_systems]
     ...
     * 
     * @param array $data Raw API response [total, codes, extra, display, code_systems]
